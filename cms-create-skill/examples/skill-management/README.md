...
| 用户在 ClawHub 找到一个 Skill，想同步到我方平台 | "把这个 ClawHub skill 同步到我们平台"、"把外部 skill 推到我们自己的平台" | publish-skill（external） | 是 |
...
- **内部 Skill**：按我方协议规范生成，通常来自新建或内部改造，发布时走打包 → 七牛上传 → 注册/更新
- **外部 Skill**：通常已存在于 ClawHub，用户目标是“同步到我方平台”，发布时直接使用 ClawHub 下载地址，不走七牛上传
- 如果用户明确提到“我在 ClawHub 找到一个 skill”“把它同步到我们平台”，优先判定为**外部 Skill**
...
1. 确认目标 Skill 已存在于 ClawHub，并拿到 `skillCode`
2. 直接构造下载地址：`https://wry-manatee-359.convex.site/api/v1/download?slug=<skillCode>`
3. 调用 `publish_skill.py --external` 或直接调用注册/更新接口
4. 确认平台已成功收录
