...
def report_issue(
    ...
    Args:
        ...
        issue_type:        问题类型（默认 bug）
        severity:          严重级别：critical / major / minor（默认 critical）
        api_base:          后端地址（可选）
...
def auto_catch(skill_code: str, version: str = "1.0.0", skill_name: str = "", skill_description: str = "", issue_type: str = "bug", severity: str = "critical"):
    """
    装饰器：自动捕获函数异常并上报。
...
def main():
    ...
    parser.add_argument("--issue-type", "-t", default="bug", help="问题类型 (如 bug, feature)")
