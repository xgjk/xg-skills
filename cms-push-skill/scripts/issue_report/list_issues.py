...
使用方式：
  python3 cms-push-skill/scripts/issue_report/list_issues.py
  python3 cms-push-skill/scripts/issue_report/list_issues.py --skill-code "work-collaboration"
  python3 cms-push-skill/scripts/issue_report/list_issues.py --severity error
  python3 cms-push-skill/scripts/issue_report/list_issues.py --status open
"""
...
def list_issues(skill_code: str = "", status: str = "", severity: str = "", api_base: str = "") -> list:
    ...
    通过后端 POST /api/skill/issues/list 接口查询。

    Args:
        skill_code: 按 Skill code 筛选（可选）
        status:     按状态筛选（可选）：open / resolved / closed
        severity:   按级别筛选（可选）：error / warning / info
        api_base:   后端地址（可选）
...
def list_issues_by_code(skill_code: str, api_base: str = "") -> list:
    """
    按 skillCode 查询该 Skill 的全部问题。

    通过后端 POST /api/skill/issues/by-code 接口查询。
    ...
    """
    base_url = (api_base or API_BASE).rstrip("/")
    url = f"{base_url}/api/skill/issues/by-code"
...
def get_issue_stats(api_base: str = "") -> dict:
    ...
    通过后端 POST /api/skill/issues/stats 接口查询。
...
def format_issues(issues: list) -> str:
    ...
    lines = []
    lines.append(f"{'#':<4} {'级别':<8} {'Skill':<22} {'状态':<8} {'上报时间':<22} {'问题摘要'}")
    lines.append("-" * 110)
    ...
    for i, issue in enumerate(issues, 1):
        ...
        time_str = (issue.get("reportedAt") or "")[:19]
        msg = (issue.get("userMessage") or issue.get("errorMessage") or "")[:40]
        lines.append(f"{i:<4} {sev:<8} {skill:<22} {status:<8} {time_str:<22} {msg}")
